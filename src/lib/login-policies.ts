...
1. 用户注册账户时须提供邮箱、用户名、密码，并完成手机号验证码校验。
2. 每个邮箱地址和手机号仅可注册一个账户，邀请码为一次性使用。
...
1. 注册信息：注册时收集邮箱地址、用户名、密码及经验证码校验的手机号，不要求真实姓名。
2. 手机号用途：手机号用于注册校验、账户安全、密码找回、DCR 准入审核和风险控制，不会向社区或互助成员公开。
