...
interface RequestEvent {
  ...
  status: number | null;
  release: string | null;
  metadata: { method?: string; requestId?: string; outcome?: string } | null;
  createdAt: string;
}
...
export default function TelemetryPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">后端请求遥测</h1><p className="mt-1 text-sm text-muted-foreground">100% 完成请求的状态、延迟、版本和规范化端点；不采集请求体、查询参数、Cookie 或密钥。</p></div><Button variant="outline" size="sm" onClick={() => void load()} disabled={loading}><RefreshCw className={`mr-1 h-4 w-4 ${loading ? "animate-spin" : ""}`} />刷新</Button></div>
