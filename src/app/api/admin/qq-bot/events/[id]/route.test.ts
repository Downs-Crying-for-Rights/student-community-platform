...
describe("GET QQ event detail", () => {
  beforeEach(() => {
    ...
    mocks.identityFindUnique.mockResolvedValue({ ciphertext: "x", iv: "y", authTag: "z", keyVersion: 1, user: { id: "user-1", nickname: "测试用户", email: null, role: "USER", isBanned: false, createdAt: new Date() } });
  ...
  it("decrypts, redacts, audits, and disables caching", async () => {
    ...
    expect(body.event.sender).toMatchObject({ qq: "2153912535", account: { id: "user-1" } });
