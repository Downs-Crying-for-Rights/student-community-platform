...
const mocks = vi.hoisted(() => ({ session: vi.fn(), findUnique: vi.fn(), identityFindUnique: vi.fn(), officialIdentityFindUnique: vi.fn(), decrypt: vi.fn(), audit: vi.fn() }));
vi.mock("next-auth/next", () => ({ getServerSession: mocks.session }));
vi.mock("@/lib/auth", () => ({ authOptions: {} }));
vi.mock("@/lib/prisma", () => ({ default: { qQBotEventInbox: { findUnique: mocks.findUnique }, qQIdentity: { findUnique: mocks.identityFindUnique }, qQOfficialIdentity: { findUnique: mocks.officialIdentityFindUnique } } }));
vi.mock("@/lib/qq-identity", () => ({ decryptQQIdentity: () => "2153912535", decryptQQOfficialIdentity: () => "openid_Abc-123" }));
...
describe("GET QQ event detail", () => {
  beforeEach(() => {
    ...
    mocks.officialIdentityFindUnique.mockResolvedValue(null);
  ...
  it("decrypts, redacts, audits, and disables caching", async () => {
    ...
    expect(body.event.sender).toMatchObject({ provider: "ONEBOT11", identity: "2153912535", username: "audit-user", userId: "user-1", nickname: "测试用户" });
  ...
  it("decrypts an official openid sender when only the official identity exists", async () => {
    mocks.session.mockResolvedValue({ user: { id: "root", role: "SUPER_ADMIN" } });
    mocks.identityFindUnique.mockResolvedValue(null);
    mocks.officialIdentityFindUnique.mockResolvedValue({ ciphertext: "x", iv: "y", authTag: "z", keyVersion: 1, user: { id: "user-2", username: "official-user", nickname: "官方用户", email: null, role: "USER", isBanned: false, createdAt: new Date() } });
    const { GET } = await import("./route");
    const response = await GET(request(), { params: { id } });
    const body = await response.json();
    expect(body.event.sender).toMatchObject({ provider: "QQ_OFFICIAL", identity: "openid_Abc-123", username: "official-user", userId: "user-2" });
  });
});
